 ...
 */
export async function unloadFilesSequentially(files, unloader) {
  try {
    for (const file of files) {
      console.log(`Attempting to unload file: ${file}`);
      await unloader(file); // Call the unloader function for each file
 ...
 */
export async function unloadSingleOrArray(files, unloader) {
  if (!Array.isArray(files)) {
    files = [files]; // Convert single file to an array
  }
  await unloadFilesSequentially(files, unloader);
}